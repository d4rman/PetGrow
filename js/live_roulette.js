...
function lrChipColor(a){
  if(a>=10000)return'#EF9F27';
  if(a>=5000)return'#a78bfa';
  if(a>=1000)return'#378ADD';
  if(a>=500)return'#1D9E75';
  return'#cc3333';
}

function lrGetTimeLeft(){
  if(!liveRound||liveRound.status!=='betting')return 0;
  return Math.max(0,Math.ceil((new Date(liveRound.betting_ends_at)-new Date())/1000));
}

function lrGetTimerColor(s){
  if(s>15)return'#1D9E75';
  if(s>7)return'#EF9F27';
  return'#cc3333';
}

function lrChipColor2(a){return lrChipColor(a);}
...
async function lrDoSpin(round){
  const {data:check}=await sb.from('live_roulette')
    .select('status,spin_result,spin_color,spin_at')
    .eq('id',round.id).single();
  if(check&&check.status==='spinning'){
    liveRound={...round,...check};return;
  }
  const n=Math.floor(Math.random()*37);
  const color=LR_NUM_COLORS[n]||'black';
  const {error}=await sb.from('live_roulette').update({
    status:'spinning',spin_result:n,spin_color:color,
    spin_at:new Date().toISOString(),
  }).eq('id',round.id).eq('status','betting');
  if(!error){
    liveRound={...round,status:'spinning',spin_result:n,spin_color:color,spin_at:new Date().toISOString()};
    await lrCalculateWinnings(round.id,n,color);
  } else {
    const {data:updated}=await sb.from('live_roulette').select('*').eq('id',round.id).single();
    if(updated)liveRound=updated;
  }
}

async function lrCalculateWinnings(roundId,n,color){
  const {data:bets}=await sb.from('live_roulette_bets').select('*').eq('round_id',roundId);
  if(!bets||!bets.length)return;
  for(const bet of bets){
    let won=0;const t=bet.bet_type,a=bet.amount;
    ...
    if(won>0){
      ...
      if(u)await sb.from('users').update({balance:u.balance+won,total_won:(u.total_won||0)+won}).eq('user_id',bet.user_id);
    }
  }
}
...
async function lrPlaceBet(type){
  ...
  await saveUser({balance:bal-liveSelectedChip,total_lost:(currentUser.total_lost||0)+liveSelectedChip});
  await sb.from('live_roulette_bets').insert({
    round_id:liveRound.id,user_id:currentUser.user_id,
    username:currentUser.username,bet_type:type,amount:liveSelectedChip,
  });
  lrRender();
}

async function lrPlaceBetNumber(n){await lrPlaceBet('n'+n);}

function lrSelectChip(amount){liveSelectedChip=amount;lrRender();}
...
async function lrClearBets(){
  ...
  if(total>0){
    ...
    if(liveRound){
      await sb.from('live_roulette_bets').delete()
        .eq('round_id',liveRound.id).eq('user_id',currentUser.user_id);
    }
  }
  liveBets={};lrRender();
}
  ...
  <div style="background:#080d14;min-height:100%;padding:12px;padding-bottom:120px">
    <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:10px">
      ...
      <div style="text-align:center">
        ...
        <div style="font-size:10px;color:#888">Общий стол · обновляется каждые 30с</div>
      </div>
    ...
    <div style="background:#0f0f1e;border-radius:12px;padding:12px;margin-bottom:10px;text-align:center;border:1px solid #2a2a4a">
      ${isBetting?`
        <div style="font-size:11px;color:#888;margin-bottom:4px">Ставки принимаются:</div>
        <div class="lr-timer-num" style="font-size:40px;font-weight:900;color:${lrGetTimerColor(timeLeft)};line-height:1">${timeLeft}</div>
        <div style="font-size:10px;color:#888;margin:4px 0">секунд</div>
        <div style="height:8px;background:#2a2a4a;border-radius:6px;overflow:hidden">
          <div class="lr-timer-bar" style="height:8px;background:${lrGetTimerColor(timeLeft)};border-radius:6px;width:${Math.min(100,timeLeft/30*100)}%;transition:width 1s linear"></div>
        </div>`
      :isSpinning?`
        <div style="font-size:14px;color:#EF9F27;font-weight:700;margin-bottom:6px">🎰 Крутим колесо...</div>
        <div style="font-size:11px;color:#888">Результат скоро!</div>`
      :`<div style="font-size:13px;color:#888">Загрузка раунда...</div>`}
    </div>
    ...
    <div style="margin-bottom:10px">
      <div style="font-size:11px;color:#888;margin-bottom:6px">Последние числа:</div>
      <div style="display:flex;gap:4px;flex-wrap:wrap">
        ${liveHistory.map(r=>`
          <div style="width:28px;height:28px;border-radius:50%;background:${r.spin_color==='red'?'#8B1A1A':r.spin_color==='green'?'#1a4a1a':'#1a1a1a'};border:2px solid ${r.spin_color==='red'?'#cc2222':r.spin_color==='green'?'#1D9E75':'#444'};display:flex;align-items:center;justify-content:center;font-size:9px;font-weight:700;color:#fff;flex-shrink:0">
            ${r.spin_result}
          </div>`).join('')}
    ...
    <div style="background:#0d2a0d;border-radius:12px;border:2px solid #1D9E75;padding:8px;margin-bottom:8px;${!isBetting?'opacity:0.5;pointer-events:none':''}">
      <div onclick="lrPlaceBetNumber(0)" style="height:34px;background:${liveBets['n0']?'rgba(29,158,117,0.5)':'#1a4a1a'};border:2px solid #1D9E75;border-radius:8px;display:flex;align-items:center;justify-content:center;cursor:pointer;font-size:16px;font-weight:900;color:#1D9E75;margin-bottom:4px;position:relative">
        0 ${liveBets['n0']?`<span style="position:absolute;right:6px;background:${lrChipColor(liveBets['n0'])};border-radius:8px;padding:1px 5px;font-size:9px;color:#fff">${liveBets['n0'].toLocaleString('ru')}</span>`:''}
      </div>
      <div style="display:grid;grid-template-rows:repeat(3,1fr);gap:3px;margin-bottom:4px">
        ${LR_TABLE_GRID.map(row=>`
          <div style="display:grid;grid-template-columns:repeat(12,1fr);gap:2px">
            ${row.map(n=>{
              const col=LR_NUM_COLORS[n]||'black';
              const key='n'+n;const hasBet=liveBets[key];
              return`<div onclick="lrPlaceBetNumber(${n})" style="height:30px;background:${hasBet?'rgba(127,119,221,0.6)':col==='red'?'#4a0000':'#141414'};border:1px solid ${col==='red'?'#993333':'#2a2a2a'};border-radius:4px;display:flex;align-items:center;justify-content:center;cursor:pointer;font-size:9px;font-weight:700;color:#fff;position:relative;user-select:none">
                ${n}${hasBet?`<div style="position:absolute;top:-3px;right:-3px;width:10px;height:10px;border-radius:50%;background:${lrChipColor(hasBet)};border:1px solid #fff"></div>`:''}
              </div>`;
            }).join('')}
          </div>`).join('')}
      </div>
      <div style="display:grid;grid-template-columns:repeat(3,1fr);gap:3px;margin-bottom:3px">
        ${[1,2,3].map(i=>`
          <div onclick="lrPlaceBet('col${i}')" style="height:22px;background:${liveBets['col'+i]?'rgba(239,159,39,0.3)':'rgba(255,255,255,0.04)'};border:1px solid #333;border-radius:4px;display:flex;align-items:center;justify-content:center;cursor:pointer;font-size:9px;color:#888;position:relative">
            2:1${liveBets['col'+i]?`<div style="position:absolute;top:-3px;right:-3px;width:10px;height:10px;border-radius:50%;background:${lrChipColor(liveBets['col'+i])};border:1px solid #fff"></div>`:''}
          </div>`).join('')}
      </div>
      <div style="display:grid;grid-template-columns:repeat(3,1fr);gap:3px;margin-bottom:3px">
        ${[{k:'dozen1',l:'1-12'},{k:'dozen2',l:'13-24'},{k:'dozen3',l:'25-36'}].map(b=>`
          <div onclick="lrPlaceBet('${b.k}')" style="height:24px;background:${liveBets[b.k]?'rgba(127,119,221,0.3)':'rgba(255,255,255,0.04)'};border:1px solid ${liveBets[b.k]?'#7F77DD':'#333'};border-radius:4px;display:flex;align-items:center;justify-content:center;cursor:pointer;font-size:9px;color:#888;position:relative">
            ${b.l} x3${liveBets[b.k]?`<div style="position:absolute;top:-3px;right:-3px;width:10px;height:10px;border-radius:50%;background:${lrChipColor(liveBets[b.k])};border:1px solid #fff"></div>`:''}
          </div>`).join('')}
      </div>
      <div style="display:grid;grid-template-columns:repeat(3,1fr);gap:3px">
        ${[
          {k:'low',l:'1-18'},{k:'even',l:'Чёт'},{k:'red',l:'🔴',bg:'#4a0000',bc:'#993333'},
          {k:'black',l:'⚫',bg:'#141414',bc:'#333'},{k:'odd',l:'Нечет'},{k:'high',l:'19-36'},
        ].map(b=>`
          <div onclick="lrPlaceBet('${b.k}')" style="height:26px;background:${liveBets[b.k]?'rgba(127,119,221,0.2)':b.bg||'rgba(255,255,255,0.04)'};border:1px solid ${liveBets[b.k]?'#7F77DD':b.bc||'#333'};border-radius:4px;display:flex;align-items:center;justify-content:center;cursor:pointer;font-size:9px;color:#fff;font-weight:600;position:relative">
            ${b.l} x2${liveBets[b.k]?`<div style="position:absolute;top:-3px;right:-3px;width:10px;height:10px;border-radius:50%;background:${lrChipColor(liveBets[b.k])};border:1px solid #fff"></div>`:''}
          </div>`).join('')}
      </div>
    </div>
    ...
    <div style="background:#0f0f1e;border-radius:10px;padding:10px;margin-bottom:8px;font-size:12px">
      <div style="color:#888;margin-bottom:4px">Мои ставки:</div>
  ...
  <div style="position:fixed;bottom:0;left:0;width:100%;background:rgba(8,13,20,0.97);border-top:1px solid #2a2a4a;padding:10px 12px;z-index:501;font-family:Arial,sans-serif">
    ${isBetting?`
      <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:6px">
        ...
        ${myTotalBet>0?`<button onclick="lrClearBets()" style="padding:5px 10px;background:none;border:1px solid #993C1D;border-radius:6px;color:#993C1D;font-size:11px;cursor:pointer;font-family:Arial,sans-serif">Сбросить</button>`:''}
      </div>
      ${myTotalBet>0
        ?`<div style="text-align:center;font-size:12px;color:#1D9E75">✅ Ставки приняты! Крутим через ${timeLeft} сек</div>`
        :`<div style="text-align:center;font-size:12px;color:#888">Нажми на стол чтобы поставить фишку</div>`
      }`
    :isSpinning
      ?`<div style="text-align:center;font-size:13px;color:#EF9F27;font-weight:700">🎰 Крутим! Жди результат...</div>`
      :`<div style="text-align:center;font-size:13px;color:#888">Загрузка...</div>`
    }
  </div>`;

  // Таймер обновляется каждую секунду без перерендера
  if(liveTimer)clearInterval(liveTimer);
  if(isBetting){
    liveTimer=setInterval(()=>{
      const tl=lrGetTimeLeft();
      const tn=document.querySelector('.lr-timer-num');
      const tb=document.querySelector('.lr-timer-bar');
      if(tn)tn.textContent=tl;
      if(tn)tn.style.color=lrGetTimerColor(tl);
      if(tb){tb.style.width=Math.min(100,tl/30*100)+'%';tb.style.background=lrGetTimerColor(tl);}
      if(tl<=0)clearInterval(liveTimer);
    },1000);
  }
...
function lrRunAnimation(resultNum,resultColor){
  ...
  const CELL_W=58,GAP=4,STEP=CELL_W+GAP,POOL_SIZE=80,WIN_IDX=60;
  ...
  strip.innerHTML='';strip.style.transition='none';strip.style.left='0px';
  pool.forEach(cell=>{
  ...
  setTimeout(()=>{
    ...
    setTimeout(()=>{
      liveSpinning=false;
      lrShowResult(liveRound.spin_result,liveRound.spin_color);
    },5300);
  },100);
}
...
function lrShowResult(n,color){
  ...
  if(myWon>0)saveUser({balance:bal+myWon,total_won:(currentUser.total_won||0)+myWon});
  ...
  popup.innerHTML=`
    ...
    <div style="font-size:13px;color:#888;margin-bottom:8px">${color==='red'?'🔴 Красное':color==='green'?'🟢 Зеро':'⚫ Чёрное'}</div>
